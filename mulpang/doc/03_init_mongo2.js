...
function registCoupon(){
	...
	db.shop.find().toArray(function(err, shops){
   		...
   		},
       {
        ...
        price: 6000, 
        quantity: 350, 
        saleDate: {start: getDay(-5), finish: getDay(4)}, 
        ...
        satisfactionAvg: 3.54354353
   		...
   		},
   		{
   			...
   			price: 6400, 
   			quantity: 500, 
   			saleDate: {start: getDay(-3), finish: getDay(10)}, 
...
function registPurchase(){
	...
	clog.info("5. 구매 등록 완료.");  	
}

var dateFormat = require('date-format');
function getDay(day){
	return dateFormat.asString('yyyy-MM-dd', new Date(now*1+(1000*60*60*24)*day));
}
function getTime(day, time){
	return dateFormat.asString('yyyy-MM-dd hh:mm:ss', new Date(now*1+(1000*60*60*24)*day + 1000*time));
}
